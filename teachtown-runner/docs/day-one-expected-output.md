...
Generated 2026-08-21 against the mock TeachTown stack with **mock student
...
[ts] TeachTown sign-in needs input — waiting for manual login.
>>> Log in on the TeachTown page in the browser, then press Enter here to continue...
...
Exit code **0**. At the manual-login line: type your TeachTown credentials
**into the browser window** (the runner never touches that form), finish the
sign-in, then press Enter in the terminal. Afterward: `recon/` holds the
screenshots and the plain-text roster report; fill `students` in
`config.json` with the exact display names the report shows.
...
- **enCORE asks to sign in again** (its session is separate): the same
  `TeachTown sign-in needs input` prompt appears mid-chain — log in in the
  browser, press Enter, the recon continues. Usually the TeachTown session
...
- **Session expired** (days later): the
  `TeachTown sign-in needs input — waiting for manual login.` prompt simply
  reappears once — log in in the browser, press Enter, done.
